...
SELECT ccs.compressed_chunk_id,round(ccs.uncompressed_heap_size, -6) uncompressed_heap_size,ccs.uncompressed_toast_size,round(ccs.uncompressed_index_size,-6) uncompressed_index_size, round(ccs.uncompressed_heap_size + ccs.uncompressed_toast_size + ccs.uncompressed_index_size, -5) AS uncompressed_total_size, round(l.relation_size, -6) relation_size, round(l.index_size, -6) index_size, round(l.total_size, -5) total_size FROM _timescaledb_catalog.compression_chunk_size ccs JOIN _timescaledb_catalog.chunk ch ON ch.id=ccs.compressed_chunk_id JOIN LATERAL (SELECT * FROM _timescaledb_functions.estimate_uncompressed_size(format('%I.%I',ch.schema_name,ch.table_name))) l ON true;
