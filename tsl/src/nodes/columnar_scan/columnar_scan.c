...
columnar_next(ColumnarScanState *state)
{
	...
	if (scandesc == NULL)
	{
		 ...
		 * compressed relation */
		if (scandesc->rs_nkeys == cstate->nscankeys)
		{
...
columnar_scan_begin(CustomScanState *state, EState *estate, int eflags)
{
	 ...
	 * implemenation. To ensure the base relation's scan slot type is used, we
	 * recreate the scan slot here with the slot type used by the underlying
	 * base relation. It is not necessary (or possible) to drop the existing
	 * slot since it is registered in the tuple table and will be released
						  ...
						  int parallel_workers)
{
	...
	path->parallel_aware = false; // TODO: Implement parallel supprt (parallel_workers > 0);
