...
tts_arrow_init(TupleTableSlot *slot)
{
	 ...
	 * compressed relation.
	 *
	 * It is only possible to intialize the non-compressed child slot here
...
tts_arrow_getsomeattrs(TupleTableSlot *slot, int attnum)
{
	...
	/* Check if attributes are already retreived */
	...
	{
		aslot->arrow_columns =
			MemoryContextAlloc(aslot->arrowdata_mcxt,
...
/*
 * Produce a Mimimal tuple copy from the slot Datum values.
 *
 * Exploit the non-compressed child slot to achieve this functionality since
 * it has the "right" tuple descriptor and will produce the correct tuple.
 */
