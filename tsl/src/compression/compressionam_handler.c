...
lazy_build_compressionam_info_cache(Relation rel, bool missing_compressed_ok)
{
	...
	caminfo->relation_id = ts_chunk_get_id_by_relid(rel->rd_id);
	...
	caminfo->compressed_relation_id =
		ts_chunk_get_compressed_chunk_id(caminfo->relation_id, missing_compressed_ok);
						...
						ParallelTableScanDesc parallel_scan, uint32 flags)
{
	...
	/* The count metadata column always directly follows the main table
	 * attributes */
								 ...
								 bool tupleIsAlive, void *state)
{
	...
	for (int i = 0; i < icstate->index_info->ii_NumIndexAttrs; i++)
	{
		...
		if (bms_is_member(table_attno, segmentby_cols))
		{
...
compressionam_alter_access_method_finish(Oid relid, bool to_other_am)
{
	...
	/* Finishing the convertion to compression TAM is handled in
