								  ...
								  const char *name)
{
	...
	if (slice->fd.range_start == INT64_MIN && slice->fd.range_end == INT64_MAX)
		return NULL;
	...
	if (dim->partitioning != NULL)
	{
		...
		{
			/* The dimension has a time function to compute the time value */
			Oid outfuncid = InvalidOid;
			getTypeOutputInfo(partinfo->partfunc.rettype, &outfuncid, &isvarlena);
			startdat = OidFunctionCall1(outfuncid,
										ts_internal_to_time_value(slice->fd.range_start,
																  partinfo->partfunc.rettype));
			enddat = OidFunctionCall1(outfuncid,
									  ts_internal_to_time_value(slice->fd.range_end,
																partinfo->partfunc.rettype));
		}
		else
		{
			/* Closed dimension, just use the integer rages */
			startdat = DirectFunctionCall1(int8out, Int64GetDatum(slice->fd.range_start));
			enddat = DirectFunctionCall1(int8out, Int64GetDatum(slice->fd.range_end));
		}
	}
	else
	{
		...
		startdat =
			OidFunctionCall1(outfuncid,
							 ts_internal_to_time_value(slice->fd.range_start, dim->fd.column_type));
		enddat =
			OidFunctionCall1(outfuncid,
							 ts_internal_to_time_value(slice->fd.range_end, dim->fd.column_type));
		dimdef = (Node *) colref;
	}

	/* Elide range constraint for +INF or -INF */
	if (slice->fd.range_start != INT64_MIN)
	{
	...
	if (slice->fd.range_end != INT64_MAX)
	{
