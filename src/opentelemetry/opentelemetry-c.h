...
extern "C"
{
	...
	void ts_opentelemetry_tracer_init(void);

	struct TracerProvider *ts_opentelemetry_tracer_provider_get();
	void ts_opentelemetry_tracer_provider_delete(struct TracerProvider *tp);

	struct Tracer *ts_opentelemetry_tracer_get(struct TracerProvider *tp, const char *library_name,
											   const char *version);
	void ts_opentelemetry_tracer_delete(struct Tracer *t);

	struct Span *ts_opentelemetry_span_start(struct Tracer *t, const char *name);
	void ts_opentelemetry_span_add_event(struct Span *s, const char *name);
	void ts_opentelemetry_span_set_attribute(struct Span *s, const char *key, const char *value);
