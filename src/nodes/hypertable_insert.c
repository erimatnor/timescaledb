				   ...
				   FdwRoutine *fdwroutine)
{
	...
	foreach (lc, mt->resultRelations)
	{
		...
		List *fdwprivate = NIL;
		bool is_data_node_insert = bms_is_member(i, hipath->data_node_insert_plans);
		...
		if (is_data_node_insert)
			direct_modify_plans = bms_add_member(direct_modify_plans, i);

		if (!is_data_node_insert && NULL != fdwroutine && fdwroutine->PlanForeignModify != NULL &&
...
ts_hypertable_insert_path_create(PlannerInfo *root, ModifyTablePath *mtpath)
{
	...
	List *subpaths = NIL;
	Bitmapset *data_node_insert_plans = NULL;
	Hypertable *ht = NULL;
	...
	forboth (lc_path, mtpath->subpaths, lc_rel, mtpath->resultRelations)
	{
		...
		if (ht != NULL)
		{
			...
			if (hypertable_is_distributed(ht) && ts_guc_max_insert_batch_size > 0)
			{
				 ...
				 * using the FDW API. */
				data_node_insert_plans = bms_add_member(data_node_insert_plans, i);
				subpath = ts_cm_functions->data_node_insert_path_create(root, mtpath, rti, i);
			}
			else
	...
	hipath->data_node_insert_plans = data_node_insert_plans;
