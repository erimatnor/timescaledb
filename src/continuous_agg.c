...
{
	int32 hyper_id = PG_GETARG_INT32(0);
	ContinuousAgg *cagg;
	StringInfo command;
	Hypertable *ht;
	Dimension *dim;
	...
	if (!max_isnull)
	{
		...
		maxdat = ts_time_bucket_add_one(maxdat, cagg->data.bucket_width, timetype);
		watermark = ts_time_value_to_internal(maxdat, timetype);
	}
	else
		/* Nothing materialized, so return NOBEGIN (-infinity) */
		watermark = ts_time_get_nobegin(timetype);
